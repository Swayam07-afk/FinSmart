...
### 2. User Authentication & Profile Management (`#login` / `#signup` / `#profile`)
- **Dual Identifier Sign In & Sign Up:** Users can create an account and log in using either **Email Address** OR **Indian Mobile Number** (10-digit).
- **Authentication Options:** Secure password-based authentication with visibility toggling and validation.
- **One-Click Demo Sign In:** Instant one-click demo login button as *Aarav Sharma* (`aarav@example.com` / `Password@123`) for effortless evaluation.
- **Persistent Profile & Score Syncing:** Automatically links completed financial surveys, scores, and tier diagnoses to the user's registered account.
- **Account Controls:** Forgot Password reset dialog, responsive navbar profile badge with initials, and clean Sign Out flow.

### 3. Diagnostic Scoring Engine & Profile
